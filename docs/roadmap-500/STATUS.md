...
- Sailing seasons (M46 follow-on): the edge law prices the annual-mean
  gyre and wind fields, but climate already solves seasonal wind bands —
  re-pricing the sea by month would open and close passages the way the
  monsoon did (out on one season's wind, home on the other), and the
  round-trip mean the ledger already books becomes the seasonal pair
  instead of the mirror pair. Instrument first: per-lane open-water
  fraction (measured to bound achievable asymmetry — cabotage hops and
  harbour tolls are symmetric by design, so the alive-share band reads
  cleaner conditioned on it), then the seasonal gains re-measure the
  mirror-advantage bands from the two solstice fields.

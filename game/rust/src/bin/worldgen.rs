...
fn main() {
    ...
    let total = w.fields.height.len() as f64;
    let land = w.fields.height.iter().filter(|&&h| h >= 0.0).count() as f64;
    let rivers = w.fields.flags.iter().filter(|&&f| f & calliope::world::CellFlags::RIVER.bits() != 0).count();
    let lakes = w.fields.flags.iter().filter(|&&f| f & calliope::world::CellFlags::LAKE.bits() != 0).count();
    ...
    let high = w.fields.height.iter().filter(|&&h| h > 0.5).count() as f64;
    ...
    let lonely = w.peoples.settlements.iter().filter(|s| s.connections == 0).count();
    ...
    let (bh, bw) = w.fields.height.dim();
    let mut border_land = 0usize;
    for x in 0..bw {
        border_land += (w.fields.height[[0, x]] >= 0.0) as usize + (w.fields.height[[bh - 1, x]] >= 0.0) as usize;
    }
    for y in 0..bh {
        border_land += (w.fields.height[[y, 0]] >= 0.0) as usize + (w.fields.height[[y, bw - 1]] >= 0.0) as usize;
    }
    ...
    for s in w.peoples.settlements.iter().take(5) {
        println!(
            "  {} ({}) pop={} food={} goods={:?}",
    ...
    if months > 0 {
        ...
        for e in w.chronicle.events.iter().rev().take(8) {
        ...
        let mut pops: Vec<i64> = w.peoples.settlements.iter().map(|s| s.pop).collect();
        pops.sort_unstable();
        ...
        let mut worst: Vec<&calliope::settlements::Settlement> =
            w.peoples.settlements.iter().filter(|s| s.peak > 0).collect();
        worst.sort_by(|a, b| {

...
impl Politics {
    ...
    pub fn op(&self, a: usize, b: usize) -> f64 {
        self.opinion[a * self.n + b]
    }
    pub fn op_add(&mut self, a: usize, b: usize, d: f64) {
        let v = &mut self.opinion[a * self.n + b];
...
fn towns_of<'a>(setts: &'a [Settlement], c: usize) -> Vec<usize> {
    setts
        .iter()
        .enumerate()
...
fn neighbours(setts: &[Settlement], a: usize, b: usize) -> bool {
...
fn strength(
        ...
        .chain(allies.iter().map(|&c| (c, 0.5)))
    {
        ...
        let war = socs
            ...
            .unwrap_or(1.0);
        let asab = pol.asab.get(ci).copied().unwrap_or(0.5);
        let legit = pol.legit.get(ci).copied().unwrap_or(0.7);
...
fn transfer(
    ...
    events.push(Event {
        ...
        text: format!(
            ...
            setts[idx].name, cultures[from].people, cultures[to].people, why
        ),
...
pub fn influence_map(
    ...
    for c in 0..n_cultures {
        let towns: Vec<&Settlement> =
            settlements.iter().filter(|s| s.culture == c).collect();
        if towns.is_empty() {
            continue;
        }
...
pub fn monthly(
    ...
    for c in 0..n {
        ...
        pol.asab[c] = (pol.asab[c] + up - ASAB_DECAY).clamp(0.05, 1.0);
        let target = 0.72;
        pol.legit[c] += (target - pol.legit[c]) * 0.0025;
    }
    ...
    for v in 0..n {
        let Some(suz) = pol.vassal_of[v] else { continue };
        if !alive(settlements, v) {
            pol.vassal_of[v] = None;
            continue;
        }
        ...
        if rng.gen::<f64>() < 0.0022 * pol.asab[v] * opening {
            pol.vassal_of[v] = None;
            ...
            events.push(Event {
                ...
                text: format!(
                    ...
                    cultures[v].people, cultures[suz].people
                ),
                ..Default::default()
            });
    ...
    if month.rem_euclid(12) == 3 {
        for c in 0..n {
            ...
            let treasury = socs.get(c).map(|s| s.treasury).unwrap_or(0.0);
    ...
    if pol.wars.len() < 3 {
        'outer: for a in 0..n {
            for b in 0..n {
                ...
                if pol.vassal_of[a].is_some() || pol.vassal_of[b] == Some(a) {
                    continue;
                }
                ...
                let dread = (pol.ae[b] / 45.0).clamp(0.0, 1.0);
                let appetite = 0.5 + pol.asab[a];
                let haz = 0.0010 * (1.0 + 2.2 * grudge + 1.4 * dread) * appetite;
                if rng.gen::<f64>() < haz {
                    // one war in three is sworn before a god (M3.5)
                    let war_god = cultures[a]
                        .pantheon
                        .iter()
                        .find(|g| g.domain == "war")
                        .or_else(|| cultures[a].pantheon.first());
                    ...
                    for j in 0..n {
                        ...
                        if pol.vassal_of[j].is_some() {
                            continue;
                        }
                        if pol.wars.iter().enumerate().any(|(k, w)| k != wi && w.involves(j)) {
                            continue;
                        }
                        let dreads = pol.ae[a] > 28.0 && pol.op(j, a) < -12.0;
                    ...
                    for &j in &joined {
                        ...
                        events.push(Event {
                            m: month,
                            s: cultures[j].people.clone(),
                            k: "war".to_string(),
                            text: format!(
                                "Dreading the appetite of the {}, the {} swear common cause with the {}.",
                                cultures[a].people, cultures[j].people, cultures[b].people
                            ),
                            ..Default::default()
                        });
                    }
                    ...
                    for j in 0..n {
                        if pol.vassal_of[j] == Some(b) && alive(settlements, j) {
                            pol.wars[wi].allies_b.push(j);
                        } else if pol.vassal_of[j] == Some(a) && alive(settlements, j) {
...
fn kindle_war(
    ...
    let war_ent = reg.add("war", name, month, None, -1, -1);
    let gen_a_name = naming::make_word(rng, &cultures[a].style, taken);
    let gen_b_name = naming::make_word(rng, &cultures[b].style, taken);
    let gen_a = reg.add_person(&gen_a_name, "general", month, Some(a));
    let gen_b = reg.add_person(&gen_b_name, "general", month, Some(b));
    let ca_ent = reg.find_kind("culture", &cultures[a].people).unwrap_or(-1);
    let cb_ent = reg.find_kind("culture", &cultures[b].people).unwrap_or(-1);
    events.push(Event {
        m: month,
        s: name.to_string(),
        k: "war".to_string(),
        text: format!(
            ...
            cultures[a].people, gen_a_name, cultures[b].people, gen_b_name
        ),
...
fn conduct_wars(
    ...
    for wi in 0..pol.wars.len() {
        ...
        if rng.gen::<f64>() < 0.09 {
            ...
            pol.legit[winner] = (pol.legit[winner] + 0.015).min(1.0);
            pol.legit[loser] = (pol.legit[loser] - 0.03).max(0.0);
            ...
            if let Some(fi) = field {
                ...
                if !gen_name.is_empty() {
                        ...
                        reg.get(gen).and_then(|e| e.epithets.last().cloned())
                    {
                        ...
                        coda = format!(
                            " {} of the {} held the day.",
                            gen_name, cultures[winner].people
                        );
                    }
                }
                events.push(Event {
                    ...
                    text: format!(
                        ...
                        settlements[fi].name, cultures[winner].people, cultures[loser].people, coda
                    ),
        ...
        if rng.gen::<f64>() < 0.18 {
            ...
            if !victims.is_empty() {
                ...
                if let Some(sa_) = socs.get_mut(attacker) {
            ...
            if let Some(ti) = target {
                ...
                events.push(Event {
                    ...
                    text: format!(
                        ...
                        cultures[att].people, settlements[ti].name
                    ),
                    ids: vec![pol.wars[wi].ent],
                    x: settlements[ti].x,
                    y: settlements[ti].y,
        ...
        if let Some(siege) = pol.wars[wi].siege.clone() {
            ...
            match ti {
                Some(ti) if settlements[ti].culture != siege.attacker => {
                    ...
                    // besiegers eat coin
                    if let Some(s) = socs.get_mut(att) {
                    ...
                    if rng.gen::<f64>() < 0.05 * (sdef / satt.max(1e-9)).min(2.0) {
                        ...
                        events.push(Event {
                            ...
                            text: format!(
                                ...
                                cultures[def].people, settlements[ti].name
                            ),
                            ids: vec![pol.wars[wi].ent],
                            x: settlements[ti].x,
                            y: settlements[ti].y,
                        ...
                        if progress >= 100.0 {
                            ...
                            pol.ae[att] = (pol.ae[att] + 12.0).min(100.0);
                        } else {
...
fn make_peace(
    ...
    for (gen, side) in [(war.gen_a, war.a), (war.gen_b, war.b)] {
        let people = cultures.get(side).map(|c| c.people.clone()).unwrap_or_default();
        reg.close(
            gen,
            month,
    ...
    let verdict = if margin < SCORE_TRIBUTE {
        ...
        format!(
            ...
            cultures.get(winner).map(|c| c.people.as_str()).unwrap_or("victors")
        )
    };
    ...
    if !alive(settlements, loser) {
        events.push(Event {
            ...
            text: format!(
                ...
                war.name, cultures[loser].people
            ),
            ids: vec![war.ent],
            ..Default::default()
        });
    ...
    if margin < SCORE_TRIBUTE || !alive(settlements, winner) {
        events.push(Event {
            ...
            text: format!(
                ...
                cultures[war.a].people, cultures[war.b].people, war.name
            ),
            ids: vec![war.ent],
            ..Default::default()
        });
    ...
    pol.legit[winner] = (pol.legit[winner] + 0.08).min(1.0);
    pol.legit[loser] = (pol.legit[loser] - 0.12).max(0.0);
    // defeat, felt at the frontier, hardens the losers' solidarity
    pol.asab[loser] = (pol.asab[loser] + 0.08).min(1.0);
    ...
    if margin < SCORE_CEDE {
        ...
        let lump = round2(socs.get(loser).map(|s| s.treasury * 0.35).unwrap_or(0.0));
        if let Some(s) = socs.get_mut(loser) {
            s.treasury = round2(s.treasury - lump);
        }
        if let Some(s) = socs.get_mut(winner) {
            s.treasury = round2(s.treasury + lump);
        }
        ...
        events.push(Event {
            ...
            text: format!(
                ...
                war.name, cultures[loser].people, lump.round() as i64, cultures[winner].people
            ),
            ..Default::default()
        });
        return (events, false);
    ...
    pol.ae[winner] = (pol.ae[winner] + 8.0 * take as f64).min(100.0);

    if annex {
        events.push(Event {
            m: month,
            s: cultures[loser].people.clone(),
            k: "realm".to_string(),
            text: format!(
                "{} ends in ruin for the {}: their last towns pass to the {}, and their realm is struck from the rolls.",
                war.name, cultures[loser].people, cultures[winner].people
            ),
            ids: vec![war.ent],
            ..Default::default()
        });
        // the people leave the rolls of the living realms (M6.1)
        if let Some(ce) = reg.find_kind("culture", &cultures[loser].people) {
            reg.close(
                ce,
                month,
        ...
        pol.vassal_of[loser] = None;
    } else if vassalise {
        pol.vassal_of[loser] = Some(winner);
        pol.ae[winner] = (pol.ae[winner] + 14.0).min(100.0);
        events.push(Event {
            m: month,
            s: war.name.clone(),
            k: "realm".to_string(),
        ...
        events.push(Event {
            ...
            text: format!(
                ...
                war.name, cultures[winner].people
            ),
            ..Default::default()
        });
    }
...
fn rebellion_pass(
    ...
    for c in 0..n {
        ...
        let p = 0.0030 * (1.0 - pol.asab[c]) * (1.1 - pol.legit[c]).max(0.0) * size_factor;
        if rng.gen::<f64>() >= p {
            continue;
        }
        ...
        let new_id = cultures.len();
        let nc = culture::secede(&cultures[c], new_id, rng, taken);
        let people = nc.people.clone();
        let parent_people = cultures[c].people.clone();
        cultures.push(nc);
        // the society splits: arts travel, coin is divided by heads
        let share = rebels.len() as f64 / towns.len() as f64;
        let parent_soc = socs[c].clone();
        ...
        pol.asab[new_id] = 0.85;
        pol.legit[new_id] = 0.55;
        pol.op_add(new_id, c, -65.0);
        pol.op_add(c, new_id, -65.0);
        pol.legit[c] = (pol.legit[c] - 0.10).max(0.0);
        // the new people and their first crown enter the telling (M6.1)
        let culture_ent = reg.add("culture", &people, month, Some(new_id), -1, -1);
        let ruler = chronicle::new_ruler(rng, &cultures[new_id], taken, month, reg);
        ...
        if pol.wars.len() < 3 && rng.gen::<f64>() < 0.5 {
            let name = format!("the {} Rising", cultures[new_id].name);
